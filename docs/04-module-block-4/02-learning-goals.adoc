...
[[LZ-4-4]]
==== LZ 4-4: Herausforderungen und Ansätze beim Messen in Enterprise-Umgebungen
Die Teilnehmenden

* kennen die Herausforderungen beim Messen in typischen Enterprise-Umgebungen, insbesondere in Bezug auf verteilte Komponenten und Umsysteme, Hybrid-Cloud-Szenarien und Compliance-Anforderungen.
* verstehen, dass ein differenzierter Messansatz notwendig ist, der sich nach Entwicklungsphase, Umgebung und Zielsetzung unterscheidet.
* wissen, welche Messansätze in den verschiedenen Phasen des Softwarelebenszyklus (z. B. Entwicklung, Integration, Lasttest, Produktion) geeignet sind.
* können einschätzen, wann klassische Ressourcenmetriken (z. B. CPU, Memory) ausreichen und in welchen Fällen grüne Metriken (z. B. Energieverbrauch, CO~2~-Emissionen) sinnvoll ergänzt oder priorisiert werden sollten.
...
[[LG-4-4]]
==== LG 4-4: Challenges and approaches to measurement in enterprise environments
Participants

* are familiar with the challenges of measurement in typical enterprise environments, particularly with regard to distributed components and peripheral systems, hybrid cloud scenarios and compliance requirements.
* understand that a differentiated measurement approach is necessary, varying according to the development phase, environment, and objectives.
* know which measurement approaches are suitable in the various phases of the software life cycle (e.g., development, integration, load testing, production).
* can assess when classic resource metrics (e.g., CPU, memory) are sufficient and in which cases green metrics (e.g., energy consumption, CO~2~ emissions) should be usefully supplemented or prioritized.
