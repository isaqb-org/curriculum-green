...
[[LZ-4-4]]
==== LZ 4-4: Herausforderungen beim Messen in Enterprise-Umgebungen analysieren und passende Messansätze ableiten
...
* Sie wissen, welche Messansätze in den verschiedenen Phasen des Softwarelebenszyklus (z. B. Entwicklung, Integration, Lasttest, Produktion) geeignet sind.
* Sie können einschätzen, wann klassische Ressourcenmetriken (z. B. CPU, Memory) ausreichen und in welchen Fällen grüne Metriken (z. B. Energieverbrauch, CO~2~-Emissionen) ergänzt oder priorisiert werden sollten.
* Sie kennen Lösungsstrategien, um mit den spezifischen Gegebenheiten in Enterprise-Umgebungen umzugehen, beispielsweise durch die Kombination geeigneter Messwerkzeuge.
...
* They can assess when classic resource metrics (e.g. CPU, memory) are sufficient and in which cases green metrics (e.g. energy consumption, CO~2~ emissions) should be supplemented or prioritised.
